...
export async function getAnnexes(apiClient, input) {
    try {
        ...
        if (annexList.length === 0) {
            ...
            if (parentName) {
                ...
                // 원래 법령명 매칭 필터
                const filtered = result3.list.filter((a) => {
        ...
        if (annexList.length === 0) {
            return notFoundResponse(`"${normalizedLawName}"에 대한 별표/서식이 법제처 DB에 없습니다.`, [
        ...
        // 최신본 우선 정렬
        annexList.sort((a, b) => (b.자치법규시행일자 || b.공포일자 || "").localeCompare(a.자치법규시행일자 || a.공포일자 || ""));
...
async function extractAnnexContent(annexList, annexSelector, normalizedLawName) {
    ...
    if (!fileLink) {
        return notFoundResponse(`"${annexTitle}"의 파일 링크가 법제처 응답에 포함되지 않았습니다.`, ["법령 전체 별표 목록을 다시 조회하세요: get_annexes({ lawName: '...' })"]);
    }
    // 파일 다운로드
...
// ─── 목록 포맷 (기존 동작) ────────────────────────────
function formatAnnexList(annexList, lawType, input, normalizedLawName) {
 ...
 */
function extractParentLawName(lawName) {
...
function extractSelectorNumbers(selector) {
...
function titleMatchesAnnexNumber(title, annexNumber) {
...
function isBundledAnnex(annexTitle) {
    return /별표\s*\d+\s*[~\-]\s*\d+/.test(annexTitle);
}
/** 묶음 별표 마크다운에서 특정 별표 섹션만 추출 */
function extractBundledSection(markdown, targetNum) {
    const num = parseInt(targetNum, 10);
    if (isNaN(num))
        return null;
    const pattern = new RegExp(`(##\\s*\\[별표\\s*${num}\\][\\s\\S]*?)(?=##\\s*\\[별표\\s*\\d|$)`);
 ...
 */
function filterByRelatedLawName(annexList, queryName) {
    if (annexList.length <= 1)
        return annexList;
    // 쿼리에서 단어 추출
    ...
    const scored = annexList.map((annex) => {
