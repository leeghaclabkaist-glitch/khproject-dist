...
function parseLinkageXML(xml, rootTag, itemTag) {
    ...
    while ((match = itemRegex.exec(content)) !== null) {
        ...
        const fieldRe = /<([^/>\s]+)>([^<]*)<\/\1>/g;
        let fm;
        while ((fm = fieldRe.exec(match[1])) !== null)
            fields[fm[1]] = fm[2].trim();
        // CDATA
        const cdataRe = /<([^/>\s]+)><!\[CDATA\[([\s\S]*?)\]\]><\/\1>/g;
        while ((fm = cdataRe.exec(match[1])) !== null)
            fields[fm[1]] = fm[2].trim();
...
async function handleLinkage(apiClient, input, cfg) {
    try {
...
// === 도구 함수 ===
export const getLinkedOrdinances = (apiClient, input) => handleLinkage(apiClient, input, {
    target: "lnkLs", primaryRoot: "LawSearch", fallbackRoot: "LnkLsSearch",
    title: "법령-자치법규 연계", emptyMsg: "연계 자치법규가 없습니다."
});
export const getLinkedOrdinanceArticles = (apiClient, input) => handleLinkage(apiClient, input, {
    target: "lnkLsOrdJo", primaryRoot: "LawSearch", fallbackRoot: "LnkLsOrdJoSearch",
    title: "법령-자치법규 조문 연계", emptyMsg: "조문 연계 결과가 없습니다."
});
export const getDelegatedLaws = (apiClient, input) => handleLinkage(apiClient, input, {
    target: "lnkDep", primaryRoot: "LawSearch", fallbackRoot: "LnkDepSearch",
    title: "위임법령 목록", emptyMsg: "위임법령이 없습니다."
});
export const getLinkedLawsFromOrdinance = (apiClient, input) => handleLinkage(apiClient, input, {
