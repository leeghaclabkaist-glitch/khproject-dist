...
function formatCitationLabel(c, officialName) {
...
async function verifyOne(apiClient, cite, apiKey) {
...
export async function verifyCitations(apiClient, input) {
    try {
        ...
        // 병렬 검증
        const results = await Promise.all(citations.map((c) => verifyOne(apiClient, c, input.apiKey)));
