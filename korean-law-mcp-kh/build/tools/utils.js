...
export async function parseJoCode(input) {
    try {
        let result;
...
export async function getLawAbbreviations(apiClient, input) {
    try {
        const extraParams = {};
        if (input.stdDt)
            extraParams.stdDt = String(input.stdDt);
        if (input.endDt)
            extraParams.endDt = String(input.endDt);
