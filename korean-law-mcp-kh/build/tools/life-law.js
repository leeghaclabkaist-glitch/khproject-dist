...
export const searchAiLawSchema = z.object({
    ...
    search: z.enum(["0", "1", "2", "3"]).default("0").describe("검색범위: 0=법령조문(기본), 1=법령 별표·서식, 2=행정규칙 조문, 3=행정규칙 별표·서식"),
    display: z.number().min(1).max(100).default(20).describe("페이지당 결과 개수 (기본값: 20)"),
    page: z.number().min(1).default(1).describe("페이지 번호 (기본값: 1)"),
    lawTypes: z.array(z.string()).optional().describe("법령종류 필터 (예: ['법률', '대통령령', '총리령,부령']). 지정 시 해당 종류만 반환."),
...
export function parseAiLawSearchXml(xmlText, searchType = "0") {
    const itemTag = itemTagMap[searchType] || "법령조문";
    const { totalCnt: totalCount, items: parsedItems } = parseSearchXML(xmlText, "aiSearch", itemTag, (itemContent) => {
        const extractField = (tag) => sharedExtractTag(itemContent, tag);
        const item = { 시행일자: extractField("시행일자") };
        if (searchType === "0") {
    ...
    const articleSignals = items
        ...
        if (!lawName || !item.조문제목 || !item.조문번호)
            return null;
        return {
            lawName,
            articleNo: item.조문번호,
...
function renderAiLawSearchResult(args, searchType, totalCount, parsedItems) {
    let items = parsedItems;
    // lawTypes 필터 적용 (클라이언트 사이드)
    if (args.lawTypes && args.lawTypes.length > 0 && items.length > 0) {
        const typeSet = new Set(args.lawTypes.map((t) => t.trim()));
...
export async function searchAiLawStructured(apiClient, args) {
...
export async function searchAiLaw(apiClient, args) {
    try {
