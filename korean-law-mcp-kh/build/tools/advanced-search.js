...
export const AdvancedSearchSchema = z.object({
    ...
    searchType: z.enum(["law", "admin_rule", "ordinance", "all"]).optional().default("law").describe("검색 대상: law (법령), admin_rule (행정규칙), ordinance (자치법규), all (전체)"),
...
export async function advancedSearch(apiClient, input) {
    try {
        // 키워드 분리 (공백 기준)
        ...
        const searchTargets = input.searchType === "all"
            ...
            : [input.searchType];
        const targetResults = await Promise.all(searchTargets.map(target => searchByType(apiClient, target, keywords, input, input.apiKey)));
 ...
 * 검색 대상별 검색 실행
 */
async function searchByType(apiClient, type, keywords, input, apiKey) {
    ...
    try {
        ...
        // 검색 대상별 XML 태그명 매핑
        const tagMap = { law: "law", admin_rule: "admrul", ordinance: "ordin" };
 ...
 */
function filterByAnd(results, keywords) {
 ...
 * 날짜 필터링
 */
function filterByDate(results, fromDate, toDate) {
    return results.filter(result => {
        if (!result.date)
            return false;
