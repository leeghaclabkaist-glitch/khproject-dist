...
function validationSearchResult(input) {
    return {
...
export async function validatePrecedentSearchResult(apiClient, input, options = {}) {
    ...
    const listText = input.hits
        ...
        .join("\n");
    if (groups.length > 0 && containsEveryTermGroup(listText, groups))
        return true;
    if (groups.length > 0) {
        const evidence = await fetchPrecedentEvidence(apiClient, validationSearchResult(input), {
            apiKey: options.apiKey,
            detailLimit: 1,
            full: input.attempt.search === 2 || options.full === true,
        });
        if (!evidence || evidence.isError)
            return false;
        return evidence.items.some(item => !item.isError && containsEveryTermGroup(item.text, groups));
    }
    if (containsAnyTerm(listText, terms))
        return true;
    const evidence = await fetchPrecedentEvidence(apiClient, validationSearchResult(input), {
        apiKey: options.apiKey,
        detailLimit: 1,
...
export async function fetchPrecedentEvidence(apiClient, searchResult, options = {}) {
    const hits = searchResult.hits
        .filter(hit => !!hit.id)
        .slice(0, clampDetailLimit(options.detailLimit));
    if (hits.length === 0)
        return null;
    const items = await Promise.all(hits.map(hit => fetchOnePrecedentDetail(apiClient, hit, options)));
    ...
    if (successCount > 0) {
        ...
        for (const item of items) {
            if (!item.isError && item.text)
                item.text = truncateResponse(item.text, perItem);
        }
    }
    const blocks = [
