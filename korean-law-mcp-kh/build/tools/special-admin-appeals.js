...
async function searchSpecialAppeals(apiClient, args, target, label, textToolName) {
    try {
        const extraParams = {
        ...
        for (const decc of result.items) {
            ...
            if (decc.청구번호)
                output += `  청구번호: ${decc.청구번호}\n`;
            if (decc.의결일자)
                output += `  의결일: ${decc.의결일자}\n`;
            if (decc.처분일자)
                output += `  처분일: ${decc.처분일자}\n`;
            if (decc.재결청)
                output += `  재결청: ${decc.재결청}\n`;
            if (decc.재결구분명)
                output += `  재결구분: ${decc.재결구분명}\n`;
            if (decc.행정심판재결례상세링크)
...
async function getSpecialAppealText(apiClient, args, target, label) {
    try {
...
export async function searchAppealReviewDecisions(apiClient, args) {
...
export async function getAppealReviewDecisionText(apiClient, args) {
...
export async function searchAcrSpecialAppeals(apiClient, args) {
...
export async function getAcrSpecialAppealText(apiClient, args) {
