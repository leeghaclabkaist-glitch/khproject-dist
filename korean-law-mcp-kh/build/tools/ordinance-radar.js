...
function findPurposeArticle(articles) {
...
function extractBasisLaws(purposeText, selfName) {
    ...
    while ((m = re.exec(purposeText)) !== null) {
        const name = m[1].trim();
        if (!/(법|법률|시행령|시행규칙|규정)$/.test(name))
            continue;
        if (normalizeAliasKey(name) === selfKey)
            continue;
        if (/(법|법률)$/.test(name) && !baseLaw)
            baseLaw = name; // 첫 법률을 "같은 법"의 지시대상으로
        push(name);
    }
    // 「」 밖의 "같은 법 시행령/시행규칙" 파생 (base 법률이 있을 때만)
    if (baseLaw) {
        if (/시행령/.test(purposeText))
            push(`${baseLaw} 시행령`);
        if (/시행규칙/.test(purposeText))
            push(`${baseLaw} 시행규칙`);
    }
    return result;
}
...
async function fetchLawStatus(apiClient, lawName, apiKey) {
...
export async function ordinanceRadar(apiClient, input) {
    try {
        ...
        if (!ordinSeq)
            return notFound("[NOT_FOUND] ordinSeq 또는 ordinanceName이 필요합니다.");
        ...
        const ordName = base.자치법규명 || "알 수 없음";
        const ordEff = base.시행일자 || "";
        const dept = base.담당부서명 || "";
        const govt = base.지자체기관명 || "";
        const articles = toArray(svc.조문?.조);
        ...
        const statuses = await Promise.all(parentNames.map(async (name) => {
            try {
                return await fetchLawStatus(apiClient, name, input.apiKey);
            }
            catch {
                return null;
            }
