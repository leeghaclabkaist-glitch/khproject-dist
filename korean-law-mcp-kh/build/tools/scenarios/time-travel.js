...
function diffArticles(oldList, newList) {
    ...
    for (const [key, cur] of newMap) {
        ...
        if (!old)
            added.push(cur);
        else if (old.body !== cur.body || old.title !== cur.title)
            modified.push({ old, cur });
    }
...
function pickVersion(versions, targetDate) {
...
function summarizeChange(old, cur) {
...
export async function runTimeTravelScenario(ctx) {
    const sections = [];
    const suggestedActions = [];
    const fromDate = ctx.extras?.fromDate;
    const toDate = ctx.extras?.toDate;
    ...
    const header = `시점 A: ${oldVer.efYd} 시행 (MST ${oldVer.mst}, ${oldArticles.length}개 조문)\n` +
    ...
    // 후속 액션
    suggestedActions.push(`${lawName} 신구대조표`, `${lawName} 조문별 개정이력`, `${lawName} 시행 ${newVer.efYd} 본문`);
    return { sections, suggestedActions };
}
