...
import { classifyDocument, DOC_LABELS, SEARCH_SUGGESTIONS, extractClauses, RISK_RULES, matchRule, computeRiskScore, extractAmounts, extractPeriods, detectConflicts, detectConflictsInText, } from "../lib/risk-rules.js";
// ────────────────────────────────────────
// 스키마
// ────────────────────────────────────────
...
// 핸들러
// ────────────────────────────────────────
export async function analyzeDocument(_apiClient, input) {
    ...
    if (amounts.length > 0 || periods.length > 0) {
        ...
        for (const a of amounts)
            out += `  [금액] ${a.label}: ${a.value}\n`;
        for (const p of periods)
            out += `  [기간] ${p.label}: ${p.value}\n`;
        out += `\n`;
    }
    // 리스크 항목
    if (findings.length === 0) {
        ...
        const sorted = [...findings].sort((a, b) => a.rule.severity === b.rule.severity ? 0 : a.rule.severity === "high" ? -1 : 1);
