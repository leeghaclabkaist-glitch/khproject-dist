...
function normalizeCandidate(query) {
    return query
        .replace(/[“”]/g, "\"")
        .replace(/[‘’]/g, "'")
        .replace(/\s+/g, " ")
...
function normalizeOriginalPrecedentQuery(query) {
...
function isUsefulCandidate(candidate, originalQuery) {
    const normalized = normalizeCandidate(candidate);
    if (!normalized || normalized === normalizeCandidate(originalQuery))
        return false;
    if (normalized.length < 2 || normalized.length > 40)
        return false;
    const tokens = normalized.split(/\s+/);
    if (/^(관한|대한|위한|따른|해당|관련)\s/.test(normalized))
        return false;
    if (tokens.some(token => /(에서|에게|으로|로서|로써|부터|까지)$/.test(token)))
        return false;
    return true;
}
function isGenericArticleTitle(title) {
    const normalized = normalizeCandidate(title);
    return LOW_INFORMATION_ARTICLE_TITLES.has(normalized);
}
function supportTokens(text) {
    return Array.from(new Set(normalizeCandidate(text)
        .replace(/<[^>]*>/g, " ")
        .replace(/[^\p{L}\p{N}]+/gu, " ")
        .split(/\s+/)
...
function normalizeAxisToken(token) {
    return token
        .replace(/[?？!.,]/g, "")
...
function extractedOriginalKeywords(originalQuery) {
...
function compactAxisTerm(term) {
    return term.replace(/\s+/g, "");
}
function canonicalFactAxisTerm(term) {
    const compact = compactAxisTerm(term);
    for (const axis of Object.keys(FACT_AXIS_SYNONYMS)) {
        if (compactAxisTerm(axis) === compact)
            return axis;
    }
    for (const [axis, synonyms] of Object.entries(FACT_AXIS_SYNONYMS)) {
        if (synonyms.some(synonym => compactAxisTerm(synonym) === compact))
            return axis;
    }
    return term;
}
function addLegalAxisForKeyword(axes, keyword) {
    ...
    if (TAX_DOMAIN_KEYWORDS.has(compact) || compact.includes("소득세") || compact.includes("조세") || compact.includes("과세")) {
        if (compact.includes("양도소득세"))
            addUnique(axes.legalAxis, "양도소득세");
        if (compact.includes("소득세"))
            addUnique(axes.legalAxis, "소득세");
        if (compact.includes("조세"))
            addUnique(axes.legalAxis, "조세");
        if (compact.includes("과세"))
            addUnique(axes.legalAxis, "과세");
    ...
    if (CONSTRUCTION_LEGAL_AXIS_KEYWORDS.has(compact)) {
        if (compact.includes("건설공사"))
            addUnique(axes.legalAxis, "건설공사");
        else if (compact.includes("건설"))
            addUnique(axes.legalAxis, "건설공사");
        else
...
function addFactAxisForKeyword(axes, keyword) {
    const compact = keyword.replace(/\s+/g, "");
    if (!compact)
        return;
    if (CONSTRUCTION_LEGAL_AXIS_KEYWORDS.has(compact))
...
function isKnownFactAxis(term) {
    const compact = term.replace(/\s+/g, "");
    return FACT_AXIS_PATTERNS.some(pattern => pattern.replace(/\s+/g, "") === compact);
}
function buildOriginalQueryAxes(originalQuery) {
    const axes = {
        legalAxis: [],
        factAxis: [],
    ...
    const hasDomainLegalAxis = axes.legalValidationTerms.some(term => (TAX_DOMAIN_KEYWORDS.has(term) ||
...
function scoreAiLawArticleCandidate(article, originalQuery, query, source) {
    ...
    if (contentHits >= 2 && content.includes(title))
        score += 15;
    else if (contentHits >= 2)
        score += 10;
    else if (contentHits === 1)
        score += 5;
...
function pushCandidate(out, seen, originalQuery, query, source, score, reason, options = {}) {
    ...
    out.push({
        ...
        variantKind: options.variantKind ?? (source === "case_number" ? "case_number" :
...
function originalKeywordTokens(originalQuery) {
    return extractedOriginalKeywords(originalQuery);
}
function addCaseNumberCandidate(out, seen, originalQuery, caseNumber) {
    ...
    if (!candidate)
        return;
    pushCandidate(out, seen, originalQuery, candidate, "case_number", 300, explicit ? "명시 사건번호" : "원질의 사건번호", {
        variantKind: "case_number",
        requiresResultValidation: false,
        allowSameAsOriginal: true,
    });
}
function addOriginalQueryCandidate(out, seen, originalQuery) {
    ...
    if (!useful)
        return;
    pushCandidate(out, seen, originalQuery, normalized, "original_query", 260, "원 자연어 질의 정규화", {
        variantKind: "original_query",
        requiresResultValidation: false,
        allowSameAsOriginal: true,
    });
}
function addOriginalKeywordCandidates(out, seen, originalQuery) {
    ...
    if (axes.legalAxis.length > 0 && axes.factAxis.length > 0) {
        ...
        pushCandidate(out, seen, originalQuery, `${legalPrimary} ${factPrimary}`, "original_keyword", 235, "원질의 법리축 + 사실축 결합", {
            variantKind: "original_keyword",
            requiresResultValidation: true,
            validationTermGroups,
        });
        if (axes.legalAxis.length >= 2) {
            pushCandidate(out, seen, originalQuery, `${axes.legalAxis.slice(0, 2).join(" ")} ${factPrimary}`, "original_keyword", 232, "원질의 법리축 확장 + 사실축 결합", {
                variantKind: "original_keyword",
                requiresResultValidation: true,
                validationTermGroups,
            });
        ...
        if (axes.factAxis.length >= 2 && isKnownFactAxis(axes.factAxis[1])) {
            pushCandidate(out, seen, originalQuery, `${legalPrimary} ${axes.factAxis[1]}`, "original_keyword", 230, "원질의 법리축 + 보조 사실축 결합", {
                variantKind: "original_keyword",
                requiresResultValidation: true,
                validationTermGroups,
            });
        ...
        if (factSynonym) {
            pushCandidate(out, seen, originalQuery, `${legalPrimary} ${factSynonym}`, "original_keyword", 228, "원질의 법리축 + 사실축 유사표현 결합", {
                variantKind: "original_keyword",
                requiresResultValidation: true,
                validationTermGroups,
            });
    ...
    if (tokens.length >= 2) {
        pushCandidate(out, seen, originalQuery, tokens.slice(0, 3).join(" "), "original_keyword", 165, "원질의 핵심 키워드 축약", {
            variantKind: "original_keyword",
            requiresResultValidation: true,
        });
    }
}
function addDocumentHintCandidates(out, seen, originalQuery, hints) {
    for (const hint of hints) {
        pushCandidate(out, seen, originalQuery, hint, "document_hint", 220, "문서/체인 쟁점 검색 hint", {
            variantKind: "document_hint",
            requiresResultValidation: false,
        });
    }
}
function addStructuredAiLawArticleCandidates(out, seen, originalQuery, articles) {
    for (const article of articles) {
        ...
        for (const variant of titleVariants(title)) {
            ...
            if (score < MIN_AI_ARTICLE_SCORE)
                continue;
            pushCandidate(out, seen, originalQuery, variant.query, "ai_law_article_title", score, "AI 법령검색 raw 조문 제목", {
            ...
            if (article.lawName && variant.search === 1) {
                ...
                if (lawTitleScore < MIN_AI_ARTICLE_SCORE)
                    continue;
                pushCandidate(out, seen, originalQuery, lawTitle, "ai_law_law_article_title", lawTitleScore, "AI 법령검색 raw 법령명 + 조문 제목", {
                    search: 1,
                    semanticAnchor: title,
...
function addRouteCandidates(out, seen, originalQuery, route) {
    const addParamCandidate = (value) => {
        if (typeof value !== "string")
            return;
        pushCandidate(out, seen, originalQuery, value, "router", 200, "query-router 후보", { variantKind: "router" });
    };
...
export function buildCompactLegalQueries(input) {
    const seen = new Set();
    const candidates = [];
