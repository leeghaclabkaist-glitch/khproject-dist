...
export function parseUpcomingXml(xmlText) {
    const byMst = new Map();
    const lawRegex = /<law[^>]*>([\s\S]*?)<\/law>/g;
    let m;
    while ((m = lawRegex.exec(xmlText)) !== null) {
        const c = m[1];
        if (extractTag(c, "현행연혁코드") !== "시행예정")
            continue;
        ...
        if (prev) {
            if (effDate && !prev.effDates.includes(effDate))
                prev.effDates.push(effDate);
            continue;
        }
        byMst.set(mst, {
...
export async function fetchUpcomingLaws(apiClient, query, apiKey) {
...
const fmtDate = (d) => (d.length === 8 ? `${d.slice(0, 4)}-${d.slice(4, 6)}-${d.slice(6)}` : d);
/**
 ...
 */
export function buildUpcomingNotes(hits, upcoming) {
