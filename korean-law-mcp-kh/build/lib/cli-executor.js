...
// Core: Execute Tool
// ────────────────────────────────────────
export async function executeTool(apiClient, toolName, params) {
 ...
 * 자연어 쿼리 실행 (라우팅 + 파이프라인)
 */
export async function executeNaturalQuery(apiClient, query, verbose) {
 ...
 */
export async function executeNaturalQueryJson(apiClient, query) {
    const route = routeQuery(query);
    try {
        // 날짜 범위가 있으면 검색 파라미터에 주입
        if (route.dateRange) {
 ...
 */
function extractPipelineId(searchTool, output) {
    ...
    if (mstMatch)
        return { mst: mstMatch[1] };
    const lawIdMatch = output.match(/법령ID:\s*(\d+)/);
    if (lawIdMatch)
        return { lawId: lawIdMatch[1] };
    return null;
}
/**
