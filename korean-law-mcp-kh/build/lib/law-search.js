...
export function stripNonLawKeywords(query) {
    return query.replace(NON_LAW_NAME_RE, " ").trim();
}
/** XML에서 법령 정보 파싱 */
export function parseLawXml(xmlText, max) {
...
export function scoreLawRelevance(lawName, query, queryWords) {
    let score = 0;
    // 정확 매칭: 쿼리가 법령명을 포함
 ...
 */
export async function findLaws(apiClient, query, apiKey, max = 3, searchDisplay = 20) {
