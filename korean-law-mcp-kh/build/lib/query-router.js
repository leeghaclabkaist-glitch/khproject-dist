 ...
 */
function hasProcedureIntent(query) {
...
const routePatterns = [
    ...
    // "관세법 2024 vs 2026", "관세법 2024-01-01 부터 2026-04-01 까지"
    {
        ...
        extract: (query, match) => {
            ...
            const params = { query: lawName, scenario: "time_travel" };
            const toYmd = (s) => {
    ...
    for (const pattern of sortedPatterns) {
        for (const regex of pattern.patterns) {
            ...
            if (match) {
                ...
                if (params._reroute) {
                    const rerouteTool = params._reroute;
                ...
                if (params._needsMst) {
                    const searchQuery = params._searchQuery || q;
 ...
 * 쿼리 의도 분석 결과 (디버깅/로깅용)
 */
export function explainRoute(query) {
