...
export function classifyDocument(text) {
    const scores = Object.fromEntries(Object.keys(DOC_SIGNALS).map(k => [k, 0]));
    for (const [type, signals] of Object.entries(DOC_SIGNALS)) {
        for (const s of signals) {
            if (text.includes(s.keyword))
                scores[type] += s.weight;
        }
    }
    // 상호 배타 보정
    if (scores.lease > 10)
        scores.employment -= 5;
    if (scores.employment > 10)
        scores.lease -= 5;
    if (scores.construction > 10)
        scores.lease -= 3;
    let best = "general";
    let bestScore = 3;
...
export function extractClauses(text, max) {
    const clauses = [];
    const splits = text.split(/(?=제\s*\d+\s*조(?:의\s*\d+)?)/);
    for (const chunk of splits) {
        if (clauses.length >= max)
            break;
...
export function detectConflicts(clauses) {
    const results = [];
    if (clauses.length < 2)
        return results;
    for (const rule of CONFLICT_RULES) {
        ...
        for (const c of clauses) {
            if (!matchA && rule.patternA.test(c.body))
                matchA = c;
            if (!matchB && rule.patternB.test(c.body))
                matchB = c;
        }
...
export function detectConflictsInText(text) {
    const results = [];
    for (const rule of CONFLICT_RULES) {
