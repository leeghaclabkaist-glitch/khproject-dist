...
export class LawApiClient {
    ...
    async throwIfError(response, endpoint) {
        if (!response.ok) {
            ...
            if (status === 429)
                throw new Error(`API 요청 한도 초과 (429) - 잠시 후 다시 시도하세요.`);
            if (status >= 500)
                throw new Error(`법제처 서버 오류 (${status}) - ${endpoint}`);
    ...
    async searchAdminRule(params) {
        ...
        if (params.knd)
            apiParams.append("knd", params.knd);
     ...
     * 자치법규 조회
     */
    async getOrdinance(ordinSeq, jo, apiKey) {
        const apiParams = new URLSearchParams({
            target: "ordin",
            OC: this.getApiKey(apiKey),
            type: "JSON",
    ...
    async getLawHistory(params) {
        ...
        if (params.display)
            apiParams.append("display", params.display.toString());
        if (params.page)
            apiParams.append("page", params.page.toString());
        const url = `${LAW_API_BASE}/lawSearch.do?${apiParams.toString()}`;
        const response = await fetchWithRetry(url);
