...
export function truncateResponse(text, maxSizeOrOpts = MAX_RESPONSE_SIZE) {
    let maxSize;
    let summary = false;
 ...
 */
function _extractSummary(text, maxSize) {
 ...
 */
export function truncateSections(text, totalMax = MAX_RESPONSE_SIZE, sectionMax) {
    if (text.length <= totalMax)
        return text;
    // "▶ " 패턴으로 섹션 분리
