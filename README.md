...
### v0.0.8
- 목차가 있는 일부 규정에서 조문이 목차 줄로 잘못 저장되던 문제 수정
  (부설 신속시범사업·방위산업기술보호 등). 실제 본문으로 재색인.
- 정상 PDF인데 조문 페이지가 누락되던 규정(무기체계 시제업무방침) 페이지 복원.

### v0.0.7
- get_rule_full 호출 실패 수정(도구 설명 문자열의 중괄호가 내부 포맷과 충돌하던 문제).
